...
struct Table {
    static var TutTable: SQLiteTable {
        var tutTable = SQLiteTable(tableName: "tutorial")
        tutTable.addColumn(columnName: "id", dataType: .INT, constraints: .PRIMARYKEY, .NOTNULL)
        tutTable.addColumn(columnName: "name", dataType: .CHAR, constraints: nil)
...
class EasySQLiteTut {
    func testWesStuff() {
        ...
        let filePath = DatabaseAccess.getDatabaseFilePath(name: "TestDatabase", pathDirectory: .documentDirectory, domainMask: .userDomainMask)
...
//---------------------------------
        ...
        var joinTable = SQLiteTable(tableName: "join")
...
//---------------------------------
        ...
        whereStatement.addStatement(table: tutTable, columnName: "name", expression: .EQUALS, columnValue: "Wes")
        whereStatement.addStatement(table: tutTable, clause: .OR, columnName: "name", expression: .EQUALS, columnValue: "Mark")
        whereStatement.addStatement(table: tutTable, clause: .OR, columnName: "id", expression: .BETWEEN, columnValue: 53, 87)
...
//---------------------------------
        var updateStatement = UpdateStatement(table: tutTable)
...
//---------------------------------
        var deleteStatement = DeleteStatement(table: tutTable)
...
//---------------------------------
        var joinStatement = JoinStatement(table1: tutTable, joinType: .INNER, table2: joinTable, onColumnName1: "name", onColumnName2: "name")
...
//---------------------------------
        ...
        selectStatement.specifyColumn(table: tutTable, columnName: "name", asName: "Person")
